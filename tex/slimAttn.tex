...
\author{Nils Graef\thanks{\texttt{info@openmachine.ai}}, \, Kayvan Zahiri, \, Andrew Wasielewski \\
...
Equation (\ref{eq6}) is exact, but in practice V is calculated at finite precision, and V can lose accuracy if $W_K$ is badly conditioned, which the determinant does not reveal. The V-cache scheme of appendix \ref{sec:vcache}, which calculates K from V, is an alternative. However, that scheme does not support RoPE. Section \ref{sec:experiments} compares both schemes in float32 for the decoder cross-attention of Whisper.
...
\section{Alternative scheme: \emph{all you need is V-cache}} \label{sec:vcache}
...
\section{Support for bias} \label{sec:bias}
