...
\begin{abstract}
Normalization introduces dependencies and intermediate operations into transformer
inference. We present \textbf{FlashNorm}, a set of algebraic transformations that
simplify normalization without retraining. Normalization gains can be folded into
adjacent linear weights, and per-token RMS scaling can be deferred until after matrix
multiplication, making the reduction and projection independent. These identities
hold in real arithmetic, including with epsilon-regularized RMS. We further
characterize when an upstream RMSNorm can be cancelled: every affected path must
pass through a downstream normalization after a bias-free, positively homogeneous
transformation. Cancellation is exact for unregularized RMS; we derive its
epsilon-dependent discrepancy for practical implementations. This distinction
enables full pre-attention cancellation in eligible QKV-normalized architectures
and partial cancellation in multi-head latent attention, where bypassing branches
must retain the scalar. On Gemma-4-E2B and Gemma-4-12B, cancellation changes
evaluated fp32 token perplexity by less than 0.001\% and preserves greedy
continuations on the tested prompts. Experiments on MiniCPM3 and DeepSeek-V2-Lite
validate partial cancellation and demonstrate severe perplexity degradation from
naive full removal. Controlled decode ablations distinguish the effects of fused
normalization kernels from weight removal. FlashNorm provides explicit conditions
and reference transformations for simplifying transformer normalization while
identifying the numerical and implementation constraints that govern practical
performance.
\end{abstract}
...
Normalization is central to the stability and generalization of modern LLMs.
RMSNorm~\citep{rms}, used by Llama~\citep{LLaMA},
...
This ordering creates a hardware dependency. The normalized activations are the
input to the linear layer, so the RMS value must be computed before the matrix unit
can begin. The matrix unit therefore waits during the reduction, and this delay is
repeated across layers and inference steps.

FlashNorm removes this dependency through two mathematically equivalent
transformations of the RMSNorm-then-project pipeline in Fig.~\ref{fig1}(a):
...
\begin{enumerate}
  \item \textbf{Weightless normalization.} The per-channel normalization weights
        ...
        so they no longer need to be stored as a separate parameter tensor (Fig.~\ref{fig1}(b)).
  \item \textbf{Deferred normalization.} Because scaling by $1/\mathrm{RMS}(\mathbf{a})$
        ...
        moved to the \emph{output} of the linear layer (Fig.~\ref{fig1}(c)).
        The matrix multiplication and RMS reduction can then run concurrently on
        separate hardware units.
\end{enumerate}

The name is inspired by FlashAttention \citep{flash-attention}, which similarly
defers normalization past a matrix multiplication to enable online tiled
computation.

FlashNorm requires no retraining: weight folding can be applied after training to
an existing checkpoint. We also show how the same ideas extend to Layer Normalization~\citep{layerNorm},
...
\begin{itemize}
  ...
  \item We show that RMS scale invariance allows an RMSNorm--linear--RMSNorm
        ...
        whenever every consumer of its output is re-normalized downstream,
  ...
  \item We verify lossless weight folding on SmolLM2-135M, Llama-3.2-1B, and
  ...
  \item We identify the kernel-engineering work needed to extend the approach to
...
Fusing small operations into one GPU kernel reduces memory traffic and kernel-launch
overhead. \citet{openelm} report that changes to layer-normalization kernels can
substantially affect tokens-per-second throughput. FlashNorm complements this line
of work: folding removes the separate weight application, while scalar deferral
exposes parallel work for the vector and matrix units.
Operation-reordering and epilogue-fusion systems (\citet{opFusion}, CODA~\citep{coda},
Mirage~\citep{mirage}) implement related deferrals at the kernel level while leaving
the stored model unchanged. Appendix~\ref{app:additional-notes} compares these
approaches with FlashNorm.
...
\end{proof}
\paragraph{QKV-normalization.}
Some models, including Gemma~4 \citep{gemma4}, apply QKV-normalization~\citep{qkv-norm} to
queries, keys, and values. Their sequence is:
pre-attention RMSNorm $\to$ QKV-projection $\to$ QKV-RMSNorm.
When the eligibility condition holds, Proposition~\ref{prop:rms-cancel} removes the
pre-attention RMSNorm and saves one full normalization per transformer block.
...
\paragraph{MLA latent normalization.}
In MLA, the relevant sequence is:
pre-attention RMSNorm $\to$ down-projection into latent space $\to$ latent RMSNorm,
which satisfies Proposition~\ref{prop:rms-cancel} on the latent path. The
decoupled RoPE key, however, is a slice of the same down-projection that bypasses the latent
...
multiplication and RMS calculation. LayerNorm's mean centering can likewise be folded
into the preceding linear layer, see Appendix~\ref{app:mean-centering}.
...
The accompanying Python implementation and a reproducible Colab benchmark are
available in the \verb+transformer-tricks+ repository\citep{hfFlashNorm, tricks}\footnote{\url{https://github.com/OpenMachine-ai/transformer-tricks/blob/main/notebooks/flashNorm\_gpu\_benchmark.ipynb}}.

We evaluated lossless weight folding on SmolLM2-135M \citep{smollm}, Llama-3.2-1B,
and Llama-3.1-8B \citep{LLaMA} using \verb+flashify_repo()+. At fp16 on
SmolLM2-135M, the source and folded models agree within representation precision
and produce the same greedy text. At fp32 on the two Llama models, greedy
generation also matches the source. Table~\ref{tab:quality} reports the source-to-
folded differences on wikitext, MMLU, and HellaSwag; all are within the measured
benchmark variation.
...
\paragraph{Norm cancellation.} We evaluated Proposition~\ref{prop:rms-cancel} after training
on Gemma-4-E2B and Gemma-4-12B, and evaluated the corrected MLA transform on
MiniCPM3-4B and DeepSeek-V2-Lite (Appendix~\ref{app:cancel}, Table~\ref{tab:cancel}).
In fp32, cancellation changes wikitext token perplexity by less than $0.001\%$ on
both Gemma models and leaves all 12 greedy continuations unchanged. The corrected
MLA transform remains within the fold-only rounding envelope, whereas naive deletion
raises perplexity from 8.8 to 188.9 on MiniCPM3 and from 7.5 to 618.4 on
DeepSeek-V2-Lite. A matched Llama-3.2-1B decode ablation (Table~\ref{tab:attribution})
attributes the measured throughput gain to kernel fusion ($+26\%$); removing the
per-channel multiply contributes less than 1\%.
...
\section{Conclusion}
We presented FlashNorm, a mathematically exact reformulation of the RMSNorm-then-project
pattern. It folds normalization weights into the following linear layer and moves the
per-token RMS scale after the matrix multiplication, exposing parallel work.
We also identified when scale invariance allows an RMSNorm--linear--RMSNorm sequence
to lose its first RMSNorm. This removes the pre-attention norm in eligible
QKV-normalized models such as Gemma~4. In MLA models, the decoupled RoPE-key path
requires a retained scalar; experiments on MiniCPM3-4B and DeepSeek-V2-Lite validate
this partial transform, while Gemma-4 experiments validate full cancellation.
...
In the memory-bound decode regime, stream-management overhead can erase the benefit.
Native kernel-level integration is therefore the main practical direction for future
work.
...
\paragraph{Scope.} The mathematics of FlashNorm (Propositions~\ref{prop:weightless} and~\ref{prop:commute}) is exact in both directions. Applying it after training is lossless in exact arithmetic; in our experiments, folded outputs match the source at fp32 and remain within benchmark variation at fp16. The same parameterization can also be used when training a model from scratch: removing the explicit $\mathbf{g}$ tensor and applying the RMS scalar after the matrix multiplication does not reduce the representable function class. Our experiments focus on post-hoc conversion; training-from-scratch results are left for future work.
...
\paragraph{Relationship to kernel-level deferral and fusion systems.} \citet{opFusion} defer Softmax and LayerNorm scaling past the following linear layer so the scaling factor can be computed alongside the matrix multiplication. CODA~\citep{coda} expresses memory-bound transformer operations as GEMM epilogues and, for a GEMM--RMSNorm--GEMM chain, places the row-wise scale in the second GEMM's epilogue. Mirage~\citep{mirage} searches for related fused kernels and verifies candidate programs on random inputs. These systems leave the stored model unchanged. FlashNorm complements them with a closed-form checkpoint transformation that removes parameter tensors and, when the eligibility condition in Section~\ref{sec:rms-cancel} holds, removes a normalization from the graph. A folded checkpoint can reduce epilogue work, while Gemma-style cancellation removes one norm per block altogether.
