...
Operation-reordering and epilogue-fusion systems such as the operation fusion of
\citet{opFusion}, CODA~\citep{coda}, and Mirage~\citep{mirage} realize the same
deferral at the kernel level while leaving the stored model unchanged;
Appendix~\ref{app:additional-notes} relates them to FlashNorm.
...
Proposition~\ref{prop:rms-cancel} removes the scalar $1/\mathrm{RMS}(\mathbf{a})$ only
because the second RMSNorm re-normalizes it away. The pre-attention RMSNorm can
therefore be dropped only if \emph{every} consumer of its output reaches a downstream
RMSNorm through a bias-free, positively homogeneous path (a bias-free linear layer or
a slice of one); any consumer that bypasses the downstream norm would silently change
scale. QK-only-normalized models (e.g., Gemma~3, which has no value norm) are
...
\paragraph{Norm cancellation.} We validate Proposition~\ref{prop:rms-cancel} post-hoc on Gemma-4-E2B and Gemma-4-12B (pre-attention norm removed) and the corrected MLA transform on MiniCPM3-4B and DeepSeek-V2-Lite (Appendix~\ref{app:cancel}, Table~\ref{tab:cancel}). In fp32, cancellation changes wikitext token perplexity by less than $0.001\%$ on both Gemma-4 models, within the rounding envelope of the mathematically exact fold-only control, and leaves all 12 greedy continuations unchanged; on the MLA models the corrected transform is likewise within the fold-only envelope, whereas naively deleting the norm raises perplexity from 8.8 to 189 (MiniCPM3) and from 7.5 to 618 (DeepSeek-V2-Lite). A matched decode ablation on Llama-3.2-1B (Table~\ref{tab:attribution}) attributes the end-to-end gain of a weightless fused norm kernel to kernel fusion ($+26\%$) rather than to eliding the per-channel multiply ($<1\%$); folding's benefit is parameter elimination and enabling the cancellation.
