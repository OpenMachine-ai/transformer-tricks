...
first one redundant whenever every consumer of its output is re-normalized downstream,
removing the pre-attention RMSNorm in QKV-normalized models (e.g., Gemma~4); in
MLA-models with latent normalization (e.g., DeepSeek-V2, Mistral Small 4) its weights
...
to drop the first RMSNorm entirely when every consumer of its output is re-normalized,
