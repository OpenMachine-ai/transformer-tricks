...
Ready-made checkpoints are published following the FlashNorm naming convention: [open-machine/gemma-4-E2B-FlashNorm](https://huggingface.co/open-machine/gemma-4-E2B-FlashNorm) is the strict artifact (folded norm tensors removed and the pre-attention RMSNorm cancelled in the modeling code via `flashNorm_modeling_gemma4.py`, trust_remote_code; outputs bitwise identical at bf16 to the compatibility checkpoint with runtime cancellation), and [open-machine/gemma-4-E2B-FlashNorm-test](https://huggingface.co/open-machine/gemma-4-E2B-FlashNorm-test) is the stock-loading companion (norm gains folded, tensors kept as all-ones, no custom code; `cancel_pre_attention_norms` applies to it directly).
